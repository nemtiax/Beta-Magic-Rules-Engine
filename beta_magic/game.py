...
class GameState:
    ...
    def _require_no_pending_action(self, *, allow_stack: bool = False) -> None:
        if self.pending_cast is not None:
            raise RuntimeError(
                f"choose targets for {self.pending_cast.spell.name} first"
            )
        if self.stack and not allow_stack:
    ...
    def activate_ability(
        ...
        self.temporary_creature_effects.setdefault(affected_card.id, []).append(
            ContinuousEffect(power=ability.power, toughness=ability.toughness)
        )
    ...
    def _validate_ability_activation(
        ...
        if not isinstance(ability, (ActivatedManaAbility, ActivatedPumpAbility)):
    ...
    def legal_targets_for(self, card: Card | None = None) -> list[Card]:
        ...
        if spell is None or spell.definition.target_requirement is None:
            return []
        requirement = spell.definition.target_requirement
        caster_id = (
    ...
    def _requirement_accepts_card(
        ...
        if not requirement.accepts_card(card):
            return False
    ...
    def legal_player_targets_for(
        ...
        if (
            spell is None
            or spell.definition.target_requirement is None
            or not spell.definition.target_requirement.players
        ):
            return []
    ...
    def upkeep_payment_required(self) -> bool:
        ...
        return not self.stack and self._timed_event_needs_payment()
    ...
    def _resolve_timed_event(self) -> None:
        ...
        if event.attached_permanent_id is not None:
            ...
            if (
                ...
                return
        affected_player = self.player(event.affected_player_id)
        self._deal_damage(affected_player, event.effect.amount, event.source_name)
        self.check_state_based_actions()
    ...
    def _resolve_batch(self) -> tuple[Card, ...]:
            ...
            for effect in card.definition.spell_effects:
                if isinstance(effect, DamageEffect):
                    ...
                    for recipient in recipients:
                        self._deal_damage(recipient, effect.amount, card.name)
        ...
        self._destroy_permanents(pending_destruction)
    ...
    def _deal_damage(
        self, recipient: Card | PlayerState, amount: int, source: str
    ) -> None:
        if isinstance(recipient, PlayerState):
            recipient.life -= amount
            self.events.append(
                DamageEvent(amount=amount, source=source, player_id=recipient.id)
            )
            if recipient.life <= 0:
    ...
    def _deal_combat_damage_wave(
        ...
        *,
        ...
        for attacker in self.combat.attackers:
            ...
            if attacker_has_first_strike is first_strike:
                if not blockers:
                    player_damage += max(0, self.creature_power(attacker))
                elif (
                    ...
                    for blocker, amount in allocations.get(attacker, {}).items():
                        if blocker.zone is Zone.BATTLEFIELD:
                            if KeywordAbility.TRAMPLE in self.creature_abilities(attacker):
                                ...
                                player_damage += amount - blocker_damage
                            else:
                                blocker_damage = amount
                            creature_damage[blocker] = (
                                creature_damage.get(blocker, 0) + blocker_damage
                            )
            ...
            for blocker in living_blockers:
                ...
                if not blocker.tapped and blocker_has_first_strike is first_strike:
                    creature_damage[attacker] = creature_damage.get(attacker, 0) + (
        ...
        if defender.life <= 0:
            defender.has_lost = True
        for creature, damage in creature_damage.items():
        ...
        while True:
            doomed = [
                ...
                and (
                    ...
                    or card.damage >= self.creature_toughness(card)
                )
            ]
            if not doomed:
                return
